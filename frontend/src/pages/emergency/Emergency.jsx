...
export default function Emergency() {
      ...
      <main className="emer-screen">
        <div className="emer-card">
          <div className="emer-header">
            ...
            <h1 className="emer-header-name">{t.emerNotFound}</h1>
            <p className="emer-header-subtitle">{t.emerSubtitle}</p>
          </div>
  ...
  const primaryLabel = contactPhone ? (contactName || t.emerEmergencyContact) : doctorPhone ? (doctorName || t.emerDoctor) : t.emerEmergencies;
  ...
  const secondaryLabel = contactPhone && doctorPhone ? (doctorName || t.emerDoctor) : t.emerEmergencies112;
      ...
      <div className="emer-card">
        ...
        <div className="emer-header">
          <div className="emer-header-top-row">
            <div className="emer-header-cross">✚</div>
            <div className="emer-header-badge">⚡ {t.emerAccessBadge}</div>
          </div>
          ...
          <h1 className="emer-header-name">{profile.fullName || t.emerPatient}</h1>
          <p className="emer-header-subtitle">{t.emerSubtitle}</p>
          <p className="emer-header-note">{t.emerNote}</p>
        </div>
        ...
        <div className="emer-body">
          ...
          {visibility !== 'full' && (
            <div className="emer-visibility-notice">
              ...
              <span>
                {visibility === 'minimal' && t.emerMinimalMode}
                {visibility === 'contact' && t.emerContactMode}
              </span>
            </div>
          )}
          ...
          <a href={`tel:${cleanPhoneForTel(primaryPhone)}`} className="emer-big-call-btn">
            ...
            <div className="emer-big-call-text">
              <strong>{t.emerCallEmergency}</strong>
          ...
          <div className="emer-section">
            ...
            <div className="emer-vital-row">
              ...
              <span className="emer-vital-label">{t.emerBloodType}</span>
            ...
            <div className="emer-vital-row">
              ...
              <span className="emer-vital-label">{t.emerChronicDiseases}</span>
              <span className={`emer-vital-value ${!formatList(profile.chronicDiseases) ? "emer-vital-value-muted" : ""}`}>
                {formatList(profile.chronicDiseases) || t.emerNoneKnown}
              </span>
            </div>
            <div className="emer-vital-row">
              <span className="emer-vital-icon">
              ...
              <span className="emer-vital-label">{t.emerMedications}</span>
              <span className={`emer-vital-value ${!formatList(profile.medications) ? "emer-vital-value-muted" : ""}`}>
                {formatList(profile.medications) || t.emerNoneMed}
              </span>
            </div>
            {(weight || height) && (
              <div className="emer-vital-row">
                ...
                <span className="emer-vital-label">{t.emerWeightHeight}</span>
              ...
              <a href={`tel:${cleanPhoneForTel(contactPhone)}`} className="emer-contact-call-card">
                <div className="emer-contact-call-left">
                  ...
                  <div className="emer-contact-call-info">
                    <strong>{contactName || t.emerContact}</strong>
                    <span>{t.emerPrimaryContactLabel}</span>
                  </div>
                </div>
              ...
              {secPhone && (
                <a href={`tel:${secPhone}`} className="emer-contact-call-card">
                  <div className="emer-contact-call-left">
                    ...
                    <div className="emer-contact-call-info">
                      <strong>{secName || t.emerSecondaryContact}</strong>
                      <span>{t.emerSecondaryContact}</span>
                    </div>
                  </div>
                  <div className="emer-contact-call-right">
                    <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#dc2626" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              {doctorPhone && (
                <a href={`tel:${cleanPhoneForTel(doctorPhone)}`} className="emer-contact-call-card emer-contact-call-card-blue">
                  <div className="emer-contact-call-left">
                    ...
                    <div className="emer-contact-call-info">
                      <strong>{doctorName || t.emerDoctor}</strong>
                      <span>{t.emerDoctorRef}</span>
                    </div>
                  </div>
                  <div className="emer-contact-call-right">
          ...
          <div className="emer-emergency-box">
            <div className="emer-emergency-box-left">
              <strong className="emer-emergency-box-title">{t.emerImportant}</strong>
              <p>{t.emerImportantText}</p>
            </div>
            <a href="tel:112" className="emer-emergency-box-call">
              ...
              <strong>112</strong>
              <span>{t.emerEmergencyNumber}</span>
            </a>
          </div>

          <Link to="/home" className="emer-return-btn">← {t.emerReturnApp}</Link>
        </div>

        {/* ═══ FOOTER ═══ */}
        <div className="emer-footer">
          <p>{t.emerFooter1}</p>
          <p>{t.emerFooter2} : {now}</p>
        </div>
      </div>
    </main>
  );
